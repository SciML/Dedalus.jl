...
@testset "EVP" begin
    ...
    @testset "Laplace Fourier basis=$btype N=$N T=$T" for
        ...
        @test isapprox(sort(real.(solver.eigenvalues)), sort(k .^ 2), atol = 1.0e-10)
    end
    ...
    @testset "Laplace Jacobi N=$N (a,b)=$ab T=$T sparse=$sparse" for
        ...
        @test isapprox(sort(real.(solver.eigenvalues))[1:Nmodes], k .^ 2, atol = 1.0e-10)
    end
    ...
    @testset "Laplace Jacobi first order N=$N (a,b)=$ab T=$T" for
        ...
        sol = sin.(x) ./ sin(x[1])
        @test isapprox(eigenfunction, sol, atol = 1.0e-10)
    end
